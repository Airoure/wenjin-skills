...
首次使用问津时，它会通过 GitHub CLI (`gh`) 识别当前登录账号：账号下已有私有 `wenjin-catalog` 就取回到本机，没有就创建一个私有仓库并初始化目录。无需手填仓库地址。若未安装 Git、`gh` 或尚未登录，问津会给出配置提示；登录可运行 `gh auth login`。已有本地目录可以继续使用，但在完成 GitHub 配置前不会声称已经同步。若本地非 Git 目录和远端都已有内容，问津会停下来提示合并，避免覆盖收藏。
...
其他网站的链接可以点 **手动填写**。保存后，条目会以“候选”状态写入个人目录 `~/.wenjin/catalog.md`；首次使用时会按上面的规则配置或取回目录。表单会自动尝试同步到 GitHub 私有仓库，并在页面显示成功或失败；失败时本地条目仍保留。表单只监听本机地址；收录不会安装或运行外部 Skill。GitHub 自动读取使用公开 API，遇到访问限制时仍可手动填写。
...
本仓库只维护问津的程序、Skill 规则和空白模板；个人收录目录放在 `~/.wenjin/catalog.md`。首次配置可在问津代码仓库运行 `python skills/wenjin/scripts/setup_catalog.py`，平时由 Skill 或网页自动调用。同步脚本只提交 `catalog.md`，不会强推；远端有新提交或本地有其他未推送提交时会停止并提示处理。同步失败后可运行 `python skills/wenjin/scripts/sync_catalog.py` 重试。若在多处复制安装 `wenjin`，每处的规则文件会独立变化；想保持一份规则，请把个人 Skill 目录中的 `wenjin` 链接到本仓库的 `skills/wenjin`，或每次更新后重新同步。目录中记录的是外部 Skill 的来源与自己的摘要，不是外部 Skill 的完整安装包。
